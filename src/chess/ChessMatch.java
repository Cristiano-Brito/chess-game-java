...
public class ChessMatch {
	...
	public boolean[][] possileMoves(ChessPosition sourcePosition){
		Position position = sourcePosition.toPosisition();
		validateSourcePosition(position);
		return board.piece(position).possibleMoves();
	}
	
	public ChessPiece perfomChessMove(ChessPosition sourcePosition, ChessPosition targetPosition) {
		Position source = sourcePosition.toPosisition();
		Position target = targetPosition.toPosisition();
	...
	public ChessPiece replacePromotedPiece(String type) {
		...
		Position pos = promoted.getChessPosition().toPosisition();
	...
	private void undoMove(Position source, Position target, Piece capturedPiece) {
		...
		// #specialmove castling kingside rook
		if(p instanceof King && target.getColum() == source.getColum() + 2) {
			Position sourceT = new Position(source.getRow(), target.getColum() + 3);
			Position targetT = new Position(source.getRow(), target.getColum() + 1);
		...
		// #specialmove castling queenside rook
		if(p instanceof King && target.getColum() == source.getColum() - 2) {
			Position sourceT = new Position(source.getRow(), target.getColum() - 4);
			Position targetT = new Position(source.getRow(), target.getColum() - 1);
			ChessPiece rook = (ChessPiece)board.removePiece(targetT);
			board.placePiece(rook, sourceT);
			rook.decreaseCount();
	...
	private boolean testCheck(Color color) {
		Position kingPosition = king(color).getChessPosition().toPosisition();
	...
	private boolean testCheckMate(Color color) {
		...
		for(Piece p : list) {
			...
			for(int i = 0; i<board.getRows(); i++) {
				for(int j = 0; j<board.getColums(); j++) {
					if(mat[i][j]) {
						Position source = ((ChessPiece)p).getChessPosition().toPosisition();
	...
	private void placeNewPiece(char column, int row, ChessPiece piece) {
		board.placePiece(piece, new ChessPosition(column, row).toPosisition());
